(function () {
                ...
                switch(pattern.exclude) {
                    case "directory":
                        if (typeof pattern.dir === "string") {
                            pattern.regex = new RegExp("^" + path.resolve(config.root, pattern.dir).replace(/\//g, "\\/") + "\\/");
                        }
                        this.excludedDirectories.push(pattern);
                        break;
                    case "file":
                        if (!pattern.regex) {
                            var excludeBaseDir = pattern.dir !== "." ? path.resolve(config.root, pattern.dir).replace(/\//g, "\\/") + "\\/" : "";
    ...
    var mustExcludeDirectory = function(dir, root) {
        var resolvedPath = path.resolve(root, dir);
        return function(excludedPattern) {
            return excludedPattern.regex.test(resolvedPath + "/");
        };
    };
