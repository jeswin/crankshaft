(function () {
    ...
    */
    var getExcludeDirectoryPredicate = function(dir, root, pattern) {
        var resolvedPath = resolveDirPath(dir, root);
        ...
        */
        ...
        var getDirWalker = function(pattern) {
            ...
            if (alreadyWalked.length) {
                ...
                return function*() {
                    var entries = {
                        paths: yield* walk(pattern.dir, pattern.recurse, pattern, self.excludedDirectories),
                        pattern: pattern
                    };
                    walkResult.entries = entries;
                    return walkResult;
                };
            }
        };
        ...
        pathsInPatternRoots.forEach(function(walkResult) {
            walkResult.entries.paths.forEach(function(entry) {
                if (entry.type === "dir") {
                    ...
                    addWatchedFile(entry, walkResult.entries.pattern);
                }
            });
        });
