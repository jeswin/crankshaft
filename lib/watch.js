(function () {
    ...
    var mustExclude = function(filePath) {
        return function(excludedPattern) {
            ...
            return /^\//.test(excludedPattern.dir) ?
                excludedPattern.regex.test(path.resolve(filePath)) : excludedPattern.regex.test(filePath);
        };
    };
            ...
            fs.watch(dirPath, function(ev, filename) {
                ...
                if (self.watchers[filePath]) {
                    ...
                    if (self.patterns.some(function(p) { return p.regex.test(filePath); })) {
                        var watcher = fs.watch(path.resolve(config.root, filePath), function(ev, filename) {
                            onFileChange(ev, filePath, watcher, self, config);
                        });
                        self.watchers[filePath] = watcher;
                    }
