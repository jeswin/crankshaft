(function () {
    ...
    var mustExcludeDirectory = function(dir, root) {
        return function(excludedPattern) {
            //If pattern directory is an absolute path, test resolved path instead of relative path.
            var dirPath = /^\//.test(excludedPattern.dir) ? path.resolve(root, dir) : dir;
            //Directory exclusion regexes have trailing '/'
            return excludedPattern.regex.test(/\/$/.test(dirPath) ? dirPath : dirPath + "/");
        };
    };
