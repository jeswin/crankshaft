    ...
    var getPatternMatchFunction = function(matchPath, root, patterns) {
        ...
        return function(exclusions) {
            if (isFilePath) {
                ...
                var excludedPatterns = exclusions.filter(function(e) { return e.type === "file"; });
                var excludedDirectories = exclusions.filter(function(e) { return e.type === "dir"; });
    ...
    Watch.prototype.getTasks = function*() {
        ...
        var walk = function*(dir, recurse, pattern, recursivelyExcludedDirs) {
            ...
            for (var i = 0; i < paths.length; i++) {
                ...
                if (info.isDirectory()) {
                    ...
                    if (matchFunc(recursivelyExcludedDirs).length) {
        ...
        */
        var dirWalkers = this.patterns.map(function(pattern) {
            ...
            if (matchFunc(self.excludedDirectories).length) {
        ...
        */
        var removeExclusionsFromList = function(list) {
            return list.map(function(entry) {
                ...
                var patterns = fn(self.excludedDirectories);
                return {
                    path: entry.path,
                    type: entry.type,
                    patterns: patterns
