(function () {
        ...
        patterns.forEach(function(pattern) {
            ...
            if (pattern.exclude === "directory") {
                if (typeof pattern.dir === "string") {
                    pattern.regex = new RegExp("^" + pattern.dir + "\\/");
                }
        ...
        filesInPatternRoots.forEach(function(filesInPattern) {
            filesInPattern.files.forEach(function(entry) {
                if (entry.type === "dir") {
                    ...
                    var mustExclude = function(excludedPattern) {
                        return excludedPattern.regex.test(entry.path);
                    };
                    var mustExcludeDirectory = function(excludedPattern) {
                        return excludedPattern.regex.test(path.dirname(entry.path));
                    };
                    if (!filesInPattern.pattern.nowatch &&
                        ...
                        !self.excludedDirectories.some(mustExcludeDirectory))
                    {
    ...
    Watch.prototype.startMonitoring = function(_onFileChange, config) {
        ...
        var onFileChange = function(ev, filePath, watcher, self, config) {
            var mustExclude = function(excludedPattern) {
                return excludedPattern.regex.test(filePath);
            };
            var mustExcludeDirectory = function(excludedPattern) {
                return excludedPattern.regex.test(path.dirname(filePath));
            };
            if (!self.excludedPatterns.some(mustExclude) &&
                !self.excludedDirectories.some(mustExcludeDirectory)) {
