(function () {
    ...
    Watch.prototype.getTasks = function*() {
        ...
        var patternRootFilesList = yield* coTools.parallel(dirWalkers);

        var yieldables = [];
        patternRootFilesList.forEach(function(patternRootFiles) {
            patternRootFiles.files.forEach(function(entry) {
                if (entry.type === "dir") {
                    self.watchedDirs.push(entry.path);
                } else if (entry.type === 'file') {
                    if (patternRootFiles.pattern.regex.test(entry.path)) {
                        yieldables.push(function*() {
                            yield* coTools.doYield(self.fn, self.parent, [entry.path, "change", patternRootFiles.pattern]);
                        });
