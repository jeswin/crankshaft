(function () {
    ...
    Build.prototype.startMonitoring = function*() {
        ...
        var onFileChange = function(ev, filePath, watcher, job, config) {
            if (!fileChangeEvents.concat(processedCycle).some(function(c) { return c.filePath === filePath && c.config === config; }))
        ...
        while(true) {
            ...
            while(fileChangeEvents.length) {
                ...
                if (fs.existsSync(changeNotification.filePath)) {
                    ...
                    yield* coTools.doYield(changeNotification.job.fn, changeNotification.config, [changeNotification.filePath, "change"]);
